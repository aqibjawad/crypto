...
Witdarwal.create = function (witdarwal_amount, result) {
    dbConn.query("INSERT INTO witdarwal SET authId=?, agreement=?, wallet_address=?",
...
Witdarwal.findById = function (id, result) {
  dbConn.query("Select * from witdarwal where id = ? ", id, function (err, res) {
    if (err) {
      console.log("error: ", err);
      result(err, null);
    }
...
Witdarwal.findAll = function (result) {
  dbConn.query("Select * from witdarwal", function (err, res) {
    if (err) {
      console.log("error: ", err);
      result(null, err);
    }
